...
- [ ] T029 [US3] CRITICAL: Remove the obsolete separate promotion description and rebuild the homepage quiz promotion as a compact single row containing only the small icon, `NEW!` badge, and `ARE YOU ERIC?` label while retaining its accessible invitation, 16 CSS-pixel minimum promotional body text, and 44-by-44 CSS-pixel link target in `index.html` per Constitution II and FR-002/FR-024 (contradicts)
