...
test("homepage quiz link stays within the current deployment path", () => {
  ...
  assert.equal(href, "./are-you-eric.html");
  assert.equal(
    new URL(href, "https://example.test/repository/index.html").pathname,
    "/repository/are-you-eric.html",
  );
});
...
test("reached quiz exposes a classic-script first-step submit control", () => {
  ...
  assert.match(quizHtml, /src="assets\/eric-profile\.js"/);
  assert.match(quizHtml, /src="assets\/eric-quiz\.js"/);
